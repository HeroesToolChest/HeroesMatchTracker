...
namespace HeroesStatTracker.Data.Replays.Queries
{
    ...
    {
        public static ReplayParseResult SaveAllData(Replay replay, string fileName, out DateTime replayTimeStamp, out long replayId, HeroesIcons heroesIcons = null)
        {
            using (var db = new ReplaysContext())
            {
                using (var dbTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        replayId = QueryDb.ReplaysDb.MatchReplay.CreateRecord(db, replayMatch);
                        ...
                        foreach (var player in players)
                        {
                            ...
                            if (QueryDb.ReplaysDb.HotsPlayer.IsExistingRecord(db, hotsPlayer))
                                playerId = QueryDb.ReplaysDb.HotsPlayer.UpdateRecord(db, hotsPlayer);
                            else
                                playerId = QueryDb.ReplaysDb.HotsPlayer.CreateRecord(db, hotsPlayer);
                            ...
                            else
                            {
                                ...
                                QueryDb.ReplaysDb.MatchPlayerTalent.CreateRecord(db, replayTalent);
                            ...
                            if (heroesIcons != null)
                            {
                                ...
                                foreach (var hero in playersHeroes)
                                {
                                    if (heroesIcons.HeroExists(hero.Key, false))
                                    {
                                        ...
                                        if (QueryDb.ReplaysDb.HotsPlayerHero.IsExistingRecord(db, playersHero))
                                            QueryDb.ReplaysDb.HotsPlayerHero.UpdateRecord(db, playersHero);
                                        else
                                            QueryDb.ReplaysDb.HotsPlayerHero.CreateRecord(db, playersHero);
                                    }
                                }
                            }
                        ...
                        if (replay.GameMode == GameMode.UnrankedDraft || replay.GameMode == GameMode.HeroLeague || replay.GameMode == GameMode.TeamLeague || replay.GameMode == GameMode.Custom)
                        {
                            if (replay.TeamHeroBans != null)
                            {
                                ...
                                if (replayTeamBan.Team0Ban0 != null || replayTeamBan.Team0Ban1 != null || replayTeamBan.Team1Ban0 != null || replayTeamBan.Team1Ban1 != null)
                                    QueryDb.ReplaysDb.MatchTeamBan.CreateRecord(db, replayTeamBan);
                            }
                        }
                        ...
                        if (xpTeam0 != null && xpTeam1 != null)
                        {
                            ...
                            for (int j = 0; j < xpTeam0.Count; j++)
                            {
                                ...
                                QueryDb.ReplaysDb.MatchTeamExperience.CreateRecord(db, xp);
                            }
                        }
                        ...
                        if (objTeam0.Count > 0 && objTeam0 != null)
                        {
                            foreach (var objCount in objTeam0)
                            {
                                ...
                                {
                                    Team = 0,
                                    PlayerId = player != null ? QueryDb.ReplaysDb.HotsPlayer.ReadPlayerIdFromBattleNetId(db, HeroesHelpers.BattleTags.GetBattleTagName(player.Name, player.BattleTag), player.BattleNetId) : (long?)null,
                                    ReplayId = replayId,
