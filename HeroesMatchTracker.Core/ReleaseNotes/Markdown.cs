    ...
    public class Markdown : DependencyObject
    {
        ...
        private static Regex _imageInline = new Regex(
                ...
                (                           # wrap whole match in $1
                    ...
                    \(                      # literal paren
                        [ ]*
                        ({GetNestedParensPatternWithWhiteSpace()})               # href = $3
                        [ ]*
                        (                   # $4
                        (['""])           # quote char = $5
                        (.*?)               # title = $6
        ...
        private static string _wholeList = $@"
            (                               # $1 = whole list
              (                             # $2
                [ ]{{0,{_tabWidth - 1}}}
                ({string.Format("(?:{0}|{1})", _markerUL, _markerOL)})                       # $3 = first list item marker
                [ ]+
              )
              (?s:.+?)
              (                             # $4
        ...
        private static Regex _listNested = new Regex(@"^" + _wholeList, RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);
        ...
        private IEnumerable<Block> DoHeaders(string text, Func<string, IEnumerable<Block>> defaultHandler)
        {
            ...
            return Evaluate(text, _headerSetext, m => SetextHeaderEvaluator(m), s => Evaluate(s, _headerAtx, m => AtxHeaderEvaluator(m), defaultHandler));
        }
        ...
        private IEnumerable<Inline> DoItalicsAndBold(string text, Func<string, IEnumerable<Inline>> defaultHandler)
        {
            ...
            if (StrictBoldItalic)
            {
                return Evaluate(text, _strictBold, m => BoldEvaluator(m, 3), s1 => Evaluate(s1, _strictItalic, m => ItalicEvaluator(m, 3), s2 => defaultHandler(s2)));
            }
            else
            {
                return Evaluate(text, _bold, m => BoldEvaluator(m, 2), s1 => Evaluate(s1, _italic, m => ItalicEvaluator(m, 2), s2 => defaultHandler(s2)));
            }
        }
